...
class TestStudentUpdate:
    def test_update_created_student(self, university_api_utils_admin):
        ...
        group = GroupRequest(name=faker.name())
        group_response = university_service.create_group(group_request=group)
        student_update = StudentDataGenerator.generate_student_data(
            group_id=group_response
        )
        student_update_response = university_service.update_student(
            student_id=student_create_response.id, student_request=student_update
        )
        ...
        assert student_create_response != get_updated_student_by_id, (
            f"Student wasn't updated, expected {get_updated_student_by_id} = {student_update_response}, "
            f"but get {get_updated_student_by_id} = {student_create_response}"
        )
